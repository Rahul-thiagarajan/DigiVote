...
var version5 = "abstract-provider/5.5.1";
...
var VoidSigner = class extends Signer {
  constructor(address, provider) {
    logger6.checkNew(new.target, VoidSigner);
    super();
...
var import_hash = __toModule(require_hash());
...
var version10 = "signing-key/5.5.0";
...
var version11 = "transactions/5.5.0";
...
function formatNumber(value, name2) {
  ...
  if (result.length > 32) {
    logger11.throwArgumentError("invalid length for " + name2, "transaction:" + name2, value);
  }
  return result;
}
...
var Formatter = class {
  constructor() {
    logger12.checkNew(new.target, Formatter);
...
var version13 = "strings/5.5.0";

// node_modules/@ethersproject/strings/lib.esm/utf8.js
"use strict";
var logger13 = new Logger(version13);
...
function namehash(name2) {
  if (typeof name2 !== "string") {
    logger14.throwArgumentError("invalid ENS name; not a string", "name", name2);
  }
  let current = name2;
  let result = Zeros;
  while (current.length) {
    ...
    if (partition == null || partition[2] === "") {
      logger14.throwArgumentError("invalid ENS address; missing component", "name", name2);
    }
...
var TypedDataEncoder = class {
    ...
    for (const name2 in types) {
      ...
      types[name2].forEach((field) => {
        if (uniqueNames[field.name]) {
          logger15.throwArgumentError(`duplicate variable name ${JSON.stringify(field.name)} in ${JSON.stringify(name2)}`, "types", types);
        }
  ...
  encodeType(name2) {
    ...
    if (!result) {
      logger15.throwArgumentError(`unknown type: ${JSON.stringify(name2)}`, "name", name2);
    }
    return result;
  }
  ...
  static hashDomain(domain) {
    ...
    for (const name2 in domain) {
      ...
      if (!type) {
        logger15.throwArgumentError(`invalid typed-data domain key: ${JSON.stringify(name2)}`, "domain", domain);
      }
...
var BaseProvider = class extends Provider {
  constructor(network) {
    logger18.checkNew(new.target, Provider);
    super();
    this._events = [];
  ...
  _getResolver(name2) {
    return __awaiter7(this, void 0, void 0, function* () {
      ...
      if (!network.ensAddress) {
        logger18.throwError("network does not support ENS", Logger.errors.UNSUPPORTED_OPERATION, { operation: "ENS", network: network.name });
      }
      const transaction = {
  ...
  resolveName(name2) {
    return __awaiter7(this, void 0, void 0, function* () {
      ...
      if (typeof name2 !== "string") {
        logger18.throwArgumentError("invalid ENS name", "name", name2);
      }
...
function checkError(method, error, params) {
  if (method === "call" && error.code === Logger.errors.SERVER_ERROR) {
    ...
    logger19.throwError("missing revert data in call exception", Logger.errors.CALL_EXCEPTION, {
      error,
      data: "0x"
    });
  }
...
var JsonRpcSigner = class extends Signer {
  constructor(constructorGuard, provider, addressOrIndex) {
    logger19.checkNew(new.target, JsonRpcSigner);
    super();
...
var JsonRpcProvider = class extends BaseProvider {
  constructor(url, network) {
    logger19.checkNew(new.target, JsonRpcProvider);
...
try {
  ...
  const logger32 = new Logger(version12);
  WS = function() {
...
var CloudflareProvider = class extends UrlJsonRpcProvider {
  static getApiKey(apiKey) {
    if (apiKey != null) {
      logger23.throwArgumentError("apiKey not supported for cloudflare", "apiKey", apiKey);
    }
    return null;
  }
  static getUrl(network, apiKey) {
...
function checkError2(method, error, transaction) {
  if (method === "call" && error.code === Logger.errors.SERVER_ERROR) {
    ...
    if (e && (e.message.match(/reverted/i) || e.message.match(/VM execution error/i))) {
      ...
      logger24.throwError("missing revert data in call exception", Logger.errors.CALL_EXCEPTION, {
        error,
        data: "0x"
      });
    }
  ...
  if (message.match(/insufficient funds/)) {
    logger24.throwError("insufficient funds for intrinsic transaction cost", Logger.errors.INSUFFICIENT_FUNDS, {
      error,
      method,
      transaction
    });
  ...
  if (message.match(/same hash was already imported|transaction nonce is too low|nonce too low/)) {
    logger24.throwError("nonce has already been used", Logger.errors.NONCE_EXPIRED, {
      error,
      method,
      transaction
    });
  ...
  if (message.match(/another transaction with same nonce/)) {
    logger24.throwError("replacement fee too low", Logger.errors.REPLACEMENT_UNDERPRICED, {
      error,
      method,
      transaction
    });
  ...
  if (message.match(/execution failed due to an exception|execution reverted/)) {
    logger24.throwError("cannot estimate gas; transaction may fail or may require manual gas limit", Logger.errors.UNPREDICTABLE_GAS_LIMIT, {
      error,
      method,
      transaction
    });
...
var EtherscanProvider = class extends BaseProvider {
  constructor(network, apiKey) {
    logger24.checkNew(new.target, EtherscanProvider);
    super(network);
  ...
  getBaseUrl() {
    ...
    return logger24.throwArgumentError("unsupported network", "network", name);
  }
        ...
        case "getLogs": {
          ...
          if (params.filter.topics && params.filter.topics.length > 0) {
            if (params.filter.topics.length > 1) {
              logger24.throwError("unsupported topic count", Logger.errors.UNSUPPORTED_OPERATION, { topics: params.filter.topics });
            }
            if (params.filter.topics.length === 1) {
              const topic0 = params.filter.topics[0];
              if (typeof topic0 !== "string" || topic0.length !== 66) {
                logger24.throwError("unsupported topic format", Logger.errors.UNSUPPORTED_OPERATION, { topic0 });
              }
              args.topic0 = topic0;
            }
          }
...
var version17 = "random/5.5.0";

// node_modules/@ethersproject/random/lib.esm/random.js
"use strict";
var logger25 = new Logger(version17);
var anyGlobal = null;
try {
  anyGlobal = window;
  if (anyGlobal == null) {
...
if (!crypto || !crypto.getRandomValues) {
  logger25.warn("WARNING: Missing strong random number source");
  crypto = {
    getRandomValues: function(buffer) {
      return logger25.throwError("no secure random source avaialble", Logger.errors.UNSUPPORTED_OPERATION, {
...
function randomBytes(length) {
  if (length <= 0 || length > 1024 || length % 1 || length != length) {
    logger25.throwArgumentError("invalid length", "length", length);
  }
  const result = new Uint8Array(length);
...
"use strict";
var logger28 = new Logger(version12);
...
"use strict";
var logger29 = new Logger(version12);
...
"use strict";
var logger30 = new Logger(version12);
var _nextId = 1;
...
"use strict";
var logger31 = new Logger(version12);
 ...
 * @license MIT
 */
//# sourceMappingURL=chunk-NU2BUXQJ.js.map
